...
/// </summary>
public class PackagesHandler : IIntegrationEventHandler
{
    private readonly IDrxPartyEventService _service;
    private readonly ILogger<PackagesHandler> _logger;
    ...
    public PackagesHandler(IDrxPartyEventService service,
        ILogger<PackagesHandler> logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    public async Task HandleAsync(MqIntegrationEvent @event)
    {
        using (LogContext.PushProperty("IntegrationEventContext", $"{@event.Id}-{Program.AppName}"))
        {
            ...
            await _service.FindPartyPackagesAsync(@event);
        }
    }
}
