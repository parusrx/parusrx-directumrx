...
/// </summary>
public class BatchSyncHandler : IIntegrationEventHandler
{
    private readonly IDrxPartyEventService _service;
    private readonly ILogger<BatchSyncHandler> _logger;
    ...
    public BatchSyncHandler(IDrxPartyEventService service,
                            ILogger<BatchSyncHandler> logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    public async Task HandleAsync(MqIntegrationEvent @event)
    {
        using (LogContext.PushProperty("IntegrationEventContext", $"{@event.Id}-{Program.AppName}"))
        {
            ...
            await _service.FindPartyBatchSyncAsync(@event);
        }
    }
}
