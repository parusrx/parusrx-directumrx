...
public class JobTitleHandler : IIntegrationEventHandler
{
    private readonly IDrxPartyEventService _service;
    private readonly ILogger<JobTitleHandler> _logger;
    ...
    public JobTitleHandler(IDrxPartyEventService service,
        ILogger<JobTitleHandler> logger)
    {
        _service = service;
        _logger = logger;
    }
    ...
    public async Task HandleAsync(MqIntegrationEvent @event)
    {
        using (LogContext.PushProperty("IntegrationEventContext", $"{@event.Id}-{Program.AppName}"))
        {
            ...
            await _service.FindPartyJobTitleAsync(@event);
        }
    }
}
